...
- ⚠️ ~~Google OAuth auto-approves new owner accounts~~ — **fixed 2026-09-03**: Google owner sign-ups now omit `approval_status` on INSERT, falling through to the schema default `'pending'`. All owner accounts (Google and regular) require admin approval before dashboard access.
- 🗑️ ~~Cascade-delete on stations destroys financial history (no soft-delete)~~ — **fixed 2026-09-03**: stations with booking/payment history can no longer be hard-deleted (API returns 409 with a clear message). Owners/admins must **deactivate** such stations instead, which preserves all financial history while blocking new bookings. Deactivated stations are hidden from public discovery and booking flows but remain visible (labeled) in owner/admin dashboards. Deactivation is reversible via reactivate.

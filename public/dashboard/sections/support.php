...
$stmt = $db->prepare("SELECT * FROM support_tickets WHERE user_type = 'driver' AND user_id = ? ORDER BY created_at DESC LIMIT 100");
...
<div class="card" style="padding:24px;margin-top:20px;max-width:720px;">
    <h3 style="margin-bottom:16px;"><i class="fas fa-life-ring" style="color:#007AFF;"></i> Submit a Ticket</h3>
    <div class="form-group" style="margin-bottom:14px;">
        ...
        <select id="st-category" style="width:100%;padding:10px 12px;border:1px solid var(--input);border-radius:var(--radius);font-size:14px;background:var(--card);color:var(--foreground);">
            ...
            <option value="charger">Charger problem</option>
    ...
    <button type="button" class="btn-primary" id="st-submit" style="background:#007AFF;border:none;color:#fff;padding:10px 20px;border-radius:var(--radius);font-weight:600;">Send Ticket</button>
    ...
    <?php else: foreach ($tickets as $t):
        ...
        <div style="border:1px solid var(--input);border-radius:var(--radius);padding:14px;margin-bottom:12px;">
            ...
            <?php if ($t['admin_reply'] !== null): ?>
                <div style="background:rgba(0,122,255,.08);border-left:3px solid #007AFF;padding:10px 12px;margin-top:10px;border-radius:4px;">
                    <div style="font-size:12px;font-weight:700;color:#007AFF;margin-bottom:4px;"><i class="fas fa-headset"></i> Support Team · <?php echo htmlspecialchars($t['replied_at']); ?></div>

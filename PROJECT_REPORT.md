...
- **CSRF gap closed:** both profile endpoints (driver avatar `sections/profile.php`, owner logo `owner_sections/profile.php`) were authenticated state-changing POSTs **outside** the CSRF gate (missed by the original sweep). Now `Csrf::validate()` on both — the dashboards' `csrf.js` already injects the token on the existing FormData posts, so the UI is unaffected. Proven live: valid shell token -> success; wrong token -> 403.
